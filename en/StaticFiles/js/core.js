...
$(document).ready(function(){
  ...
  // Tool buttons tip
  $('.tt').hover(function(){var d = $(this).attr('data-distance').split('|');$('.tip', $(this)).css('top',d[0]+'px');(d[2] == 'top') ? $('.tip', $(this)).addClass('t') : $('.tip', $(this)).addClass('b');centerElement($(this), '.tip', true);$(this).find('.tip').stop(true, true).removeClass('hide').animate({ 'top': d[1], 'opacity': 1 }, 200 );},function(){var d = $(this).attr('data-distance').split('|');$(this).find('.tip').stop(true, false).animate({ 'top': d[0], 'opacity': 0 }, 200, function(){$(this).addClass('hide');});});

  // Header menu
  $('.more-menu .box').hover(function(){$(this).find('ul').show();}, function(){$(this).find('ul').hide();});
  ...
  // Check prefered scripting language
  ...
  // Custom select box (history)
  $('.select-box .label').click(function(e){
    var index = $(this).parent().index();
    $.each($('.select-box .label'), function(i,l){
      if($(this).parent().index() != index) $('ul',$(this).parent()).hide();
    });
    var el = $(this).parent().find('ul');
    (el.css('display') == 'none') ? el.slideDown(150) : el.hide();
  });

  // Custom select box options (history)
  $('.select-box').on('click','li',function(e){
    ...
    $('.history-table tr.version-' + version).show();

  });

  // Select latest version in history
  if(location.href.indexOf('40_history') > 0){
    $('.history-selector ul li:first-child').trigger('click');
  }
...
function columnHeight(){
  var sidebarHeight = getHeight() - 230;
  var tocHeight = getHeight() - 330;
